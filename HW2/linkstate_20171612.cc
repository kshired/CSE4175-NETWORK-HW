    ...
    for (int start = 0; start < n; ++start)
    {
        ...
        while (!pq.empty())
        {
            ...
            for (int i = 0; i < n; ++i)
            {
                ...
                if (nxtWeight < dist[start][i])
                {
                    dist[start][i] = nxtWeight;
                    path[start][i] = now;
                    pq.push(make_pair(nxtWeight, i));
                }
            }
        }
...
int main(int argc, char *argv[])
{
    ...
    // solve
    ofstream result_file("output_ls.txt");
    solve(result_file, n);
